...
def KLD_enc_2d_L2(output, target):
    recon_x, l1_mu_logvar, l2_mu_logvar_enc, l2_mu_logvar_dec = output
    l2_mu = l2_mu_logvar_enc[:, 0:int(l2_mu_logvar_enc.size()[1]/2)]
    l2_logvar = l2_mu_logvar_enc[:, int(l2_mu_logvar_enc.size()[1]/2):]
    L2_KLD = -0.5 * torch.sum(1 + 2 * l2_logvar - l2_mu.pow(2) - (2 * l2_logvar).exp())
    return L2_KLD.item()

def KLD_dec_2d_L2(output, target):
    recon_x, l1_mu_logvar, l2_mu_logvar_enc, l2_mu_logvar_dec = output
    l2_mu = l2_mu_logvar_dec[:, 0:int(l2_mu_logvar_dec.size()[1]/2)]
    l2_logvar = l2_mu_logvar_dec[:, int(l2_mu_logvar_dec.size()[1]/2):]
    L2_KLD = -0.5 * torch.sum(1 + 2 * l2_logvar - l2_mu.pow(2) - (2 * l2_logvar).exp())
    return L2_KLD.item()
