...
class HVAE(BaseModel):
    ...
    small_vae_loc: .../model_best.pth'
    """
    def __init__(self, small_vae_loc=None, img_size=256, in_channels=1, l2_latent_size=32, l1_latent_size=32, l2_stride=32):
        ...
        if small_vae_loc: # use pretrained small vae, don't train weights
            checkpoint = torch.load(small_vae_loc)
            state_dict = checkpoint['state_dict']
            self.l2_vae.load_state_dict(state_dict)
            for p in self.l2_vae.parameters():
    ...
    def encode(self, x, deterministic=True):
        ...
        l2_mu_logvar_enc = self.l2_conv_vae.encode_vae_layer(x)
        x = self.l2_conv_vae.reparameterize_vae_layer(l2_mu_logvar_enc, deterministic=deterministic)
        l1_mu_logvar = self.l1_vae.encode(x)
        return l1_mu_logvar, l2_mu_logvar_enc
